...
const flowSteps = [
  ...
  { t: '天亮报夜', night: false, d: '法官宣布天亮，按序播报死讯并询问是否留遗言；若无人死亡则宣布「平安夜」。首夜出局的玩家与白天被放逐的玩家可以发表遗言，具体口径以当场版型规则为准。' },
  { t: '警长竞选', night: false, d: '首夜天亮后进行：想竞选的玩家举手报名（上警），上警玩家依次发言后可退水（退水者失去本轮投票权）；由未上警的玩家投票选出警长。平票时平票玩家再次发言后全场重投，二次平票则本局警徽流失。' },
  { t: '轮流发言', night: false, d: '从死者相邻位置（死左/死右）开始按序轮流，每人限时发言：报身份、盘逻辑、指认或表水，其余玩家不得插话、提问。' },
  { t: '放逐投票', night: false, d: '全员同时举票，禁止跟票、变票，得票最多者出局留遗言；平票时平票玩家进入 PK 发言后再投一次，二次平票则当日无人出局。' },
  { t: '自爆与入夜', night: true, d: '白天发言阶段狼人可随时自爆：翻牌出局并跳过当天剩余流程直接进入黑夜；警长竞选期间出现双爆（连续两只狼自爆）则警徽流失。循环往复，直到某一阵营达成胜利条件。' },
]
